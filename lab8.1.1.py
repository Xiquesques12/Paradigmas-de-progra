...
class Particula():
    def __init__(self,x:float,y:float):
        ...
        self.y=y
class Nodo():
    def  __init__(self,x0:float,y:float,w:float,h:float,particulas):
        self.x0 = x0
        self.y0 = y0
        self.ancho = w
        self.alto = h 
...
def subdivision_recursiva(mpdp:Nodo,k:int):
    if len(nodo.particulas)<=k:
        return
    
    w=float(0.5*nodo.ancho)
    h=float(0.5*nodo.alto)
    ...
    p = cuantas_cotiene(nodo.x0,nodo.y0+h_,w_,nodo.particulas)
    nodo.x2 = Nodo(nodo.x0,nodo.y0+h_,w_,p)
    ...
    nodo.x3 = Nodo(nodo.x0+w_,nodo.y0+h_,w_,h_,p)
...
def encontrar_hijos(nodo):
    if not nodo.hijos:
        ...
        for hijo in nodo.hijos:
            hijos += (encontrar_hijos(hijos))
    return hijos
class QTree():
    def __init__(self, k:int,n:int):
        self.umbral = k
        self.particulas = [Particula(random.uniform(0,10),random.uniform(o,10)) for x in range(n)]
